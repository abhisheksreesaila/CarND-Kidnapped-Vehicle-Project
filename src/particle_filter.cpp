...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  num_particles = 25;
	...
	for (i = 0; i < num_particles; i++) {
	  ...
	  current_particle.x = dist_x(drand);
	  current_particle.y = dist_y(drand);
	  current_particle.theta = dist_theta(drand);
...
void ParticleFilter::prediction(double delta_t, double std_pos[], 
   ...
   */
	...
	for (int i=0; i < num_particles; i++) {
	  ...
	  particles[i].x = dist_x(drand);
	  particles[i].y = dist_y(drand);
	  particles[i].theta = dist_theta(drand);
	}
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    weight_adjust += particles[i].weight;
  }

  // Normalize the weights of all particles since resmapling using probabilistic approach
  for (int i = 0; i < particles.size(); i++) {
    particles[i].weight /= weight_adjust;
